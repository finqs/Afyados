...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const allowed = ['enunciado','alternativa_a','alternativa_b','alternativa_c','alternativa_d','alternativa_e','gabarito','comentario','area','apg_numero','tem_imagem','imagem_descricao']
  ...
  const { error } = await sb.from('questoes').update(update).eq('id', params.id)
  if (error) {
    console.error('questoes update error:', error.message)
